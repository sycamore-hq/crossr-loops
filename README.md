...
**Dogfood is live (split-08).** First published pin: [`v0`](https://github.com/sycamore-hq/crossr-loops/releases/tag/v0). Current consumer pins: `skills = "v1-one-law"`, `loops = "v1-cards"` ([lockfile.toml](lockfile.toml)). Process files came from `crossr-harness --process-only` — they are a consumer instance, not loop law.
