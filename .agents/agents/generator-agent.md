...
5. When the write is ready for Reviewer (Tester `BLESS` on this SHA, or the brief says hand off): if no Draft PR exists yet, open one now (`gh pr create --draft`), then mark it Ready for review (`gh pr ready`). That is the handoff to Reviewer, not "the human may merge." A Tester `REJECT` is step 6, not a handoff.
