...
Required Skills named in a persona resolve at `/workspace/.crossr/skills/<pin>/.agents/skills/<name>/SKILL.md` first, else `/workspace/.crossr/loops/<pin>/.agents/skills/<name>/SKILL.md` (loop cards such as `avril` live there). Briefs include `SKILLS: skills <pin> /workspace/.crossr/skills/<pin>/.agents/skills/` and the loops fallback `/workspace/.crossr/loops/<pin>/.agents/skills/`. The seat reads each named SKILL.md from those roots in that order. Missing in both, or a Bot that cannot load it → that seat cards you. You ask the user. Do not skip the skill. Do not paste skill bodies into Descriptions.
...
At AVRIL start, create the `Blessed Backlog Summary` issue on the active repo if it does not exist (body: `in progress`). Every PO / QA / CTO verdict line (`SEAT: <seat>` plus `BLESS <id>` or `REJECT <id> — <blockers>`) is posted as a comment on that issue. Those comment URLs are the witnesses.
...
When every active PBI has three fresh child-authored BLESS marks on GitHub, write the Blessed Backlog Summary as the body of that same issue (update it). Include the witness URL for each BLESS. Quote that issue URL here. Stop.
...
Generator opens a Draft PR only as a durable save point. At Reviewer handoff, if no Draft exists yet, the Generator brief's `DO:` names `gh pr create --draft` then `gh pr ready`. Those verbs live in the brief, not in generator-agent.md. Ready requires a child-authored Tester BLESS witness on that SHA. You do not run gh pr create / ready / merge unless the user instructed that verb this turn. After Reviewer BLESS plus the repo's named check transcript (usually `just check`), announce that the PR is ready to merge and nudge the user. Do not merge. No named recipe → card. Do not invent a check. The user merges.
