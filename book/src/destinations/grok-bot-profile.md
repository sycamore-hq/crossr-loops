...
- No lockfile.toml → say so. Tell the user you will run CrossR harness bootstrap on this repo. Ask yes. Silence = stop. Clone sycamore-hq/crossr-harness at its default branch into `/workspace/.crossr/harness` unless the user names a tag. Run `scripts/harness-bootstrap` from that checkout against `/workspace/<repo>`. Show the command before you run it. Do not pass invented `skills=` / `loops=` flags. Show the pins the tool wrote. Then check out loops at the new `loops` pin into `/workspace/.crossr/loops/<pin>/`. If bootstrap fails or wants a flag you do not know, card and stop. Do not invent pins. Do not write lockfile.toml by hand.
- Repo already has lockfile.toml → still ensure `/workspace/.crossr/loops/<pin>/` exists for that repo's pin.
...
Order per unit: Generator plan → Architect BLESS → Generator code on the unit branch → mechanical (you run the repo's named check on that SHA on this computer; red → Generator, no LLM verdict) → Tester BLESS → Ready → Reviewer BLESS. On REJECT follow axel.md: Tester → mechanical + Tester; Reviewer → mechanical + Reviewer.
