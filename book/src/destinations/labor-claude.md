...
### 6. Spawn pattern
New empty process every job. Load oauth first. Write the brief to a file; do not inline it. Each spawn uses its own `D="$(mktemp -d /tmp/labor.XXXXXX)"` for the brief and, on review, the scratch worktree and `review.json`. Review spawns stay non-mutating on `/workspace`. Do not `gh pr checkout` the shared tree. Fix spawn is mutating: edits and pushes the unit branch only; still no merge. A fix run ends with a commit pushed on the unit branch; report the HEAD SHA (rule 5).
...
BRIEF
printf 'Draft the review to %s. Post it with `gh api repos/<owner>/<repo>/pulls/<n>/reviews --input %s` (no -X flag). Scratch worktree at %s/wt; diff at %s/pr.diff.\n' "$D/review.json" "$D/review.json" "$D" "$D" >> "$D/brief.md"
claude -p "/github-pr-review $(cat "$D/brief.md")" --allowedTools "Bash(gh pr view *),Bash(gh pr diff *),Bash(gh pr checks *),Bash(gh api graphql *),Bash(gh api repos/*/pulls/*),Bash(gh api repos/*/compare/*),Bash(gh api user *),Edit(//${D#/}/**),Write(//${D#/}/**),Bash(git fetch origin *),Bash(git worktree add $D/* *),Bash(git worktree remove *),Bash(python3 /home/box/.claude/skills/github-pr-review/scripts/validate_review.py *)" --disallowedTools "NotebookEdit,Bash(git push*),Bash(gh pr merge*),Bash(gh api *merge*),Bash(gh api *AutoMerge*),Bash(gh api *createCommitOnBranch*),Bash(gh api *updateRef*),Bash(gh api *createRef*),Bash(gh api *-X*),Bash(gh api *--method*),Bash(gh api *update-branch*),Bash(gh api *dismissals*),Bash(gh api *markPullRequestReadyForReview*),Bash(gh api *closePullRequest*),Bash(gh api *dismissPullRequestReview*),Bash(gh api *deleteRef*)" --output-format text </dev/null
```
...
```bash
. /home/box/.config/claude/load-oauth.sh
cd /workspace/<repo> && git fetch origin <unit-branch> && git checkout <unit-branch> && git merge --ff-only origin/<unit-branch> || { echo "unit branch not checked out; card"; exit 1; }
D="$(mktemp -d /tmp/labor.XXXXXX)"
cat > "$D/brief.md" <<'BRIEF'
<paste the brief verbatim>
BRIEF
claude -p "/github-pr-fix $(cat "$D/brief.md")" --permission-mode acceptEdits --allowedTools "Bash(git add *),Bash(git commit *),Bash(git push origin <unit-branch>),Bash(git push -u origin <unit-branch>),Bash(git status*),Bash(git diff*),Bash(git log*),Bash(git show*),Bash(git fetch origin <unit-branch>),Bash(git pull --ff-only origin <unit-branch>),Bash(gh pr view *),Bash(gh pr diff *),Bash(gh pr checks *),Bash(gh api graphql *),Bash(gh api repos/*/pulls/*/comments/*),Bash(gh api repos/*/compare/*),Bash(gh api user *)" --disallowedTools "Bash(gh pr merge*),Bash(gh api *merge*),Bash(gh api *AutoMerge*),Bash(gh api *createCommitOnBranch*),Bash(gh api *updateRef*),Bash(gh api *createRef*),Bash(git push origin main*),Bash(gh api *-X*),Bash(gh api *--method*),Bash(gh api *update-branch*),Bash(gh api *dismissals*),Bash(gh api *markPullRequestReadyForReview*),Bash(gh api *closePullRequest*),Bash(gh api *dismissPullRequestReview*),Bash(gh api *deleteRef*)" --output-format text </dev/null
```
